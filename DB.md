...
docker compose down
docker run --rm --mount type=volume,src=henhouse_server_db_data,dst=/var/lib/postgresql -e POSTGRES_PASSWORD=<PG_ADMIN_PASSWORD> -e PGAUTO_ONESHOT=yes pgautoupgrade/pgautoupgrade:<NEW_PG_VERSION>-alpine
git pull
docker compose up -d
```
